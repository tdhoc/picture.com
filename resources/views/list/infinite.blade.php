            ...
            <div class="boxcaption">
                <span class='thumb-info-big'>
                    <span>{{$picture['resolution']}}</span>&nbsp;&nbsp;
                        <a href="{{ asset('by_category/'.$category['id'].'/1') }}" title="{{$category['name']}} HD Wallpapers | Background Images">{{$category['name']}}</a>&nbsp;&nbsp;
                        <a href="{{ asset('by_subcategory/'.$picture['subcategory']['id'].'/1') }}" title="{{$picture['subcategory']['name']}} HD Wallpapers | Background Images">{{$picture['subcategory']['name']}}</a>
                    </span>
                    <br/>
                    <div class="overlay">
                            <div>
                    ...
                    success : function (data){
                        if(data.error == true){
                            ...
                            $.each(data.picturelist, function(key, picture) {
                                $picturehtml = '<div class="thumb-container-big" id="' + picture["id"] +'"><div class="thumb-container"><div class="boxgrid"><a href="/images/' + picture["id"] +'" title="' + $category["name"] + '" "' + picture["subcategory"]["name"] + '" "' + picture["tagstring"] + ' HD Wallpaper | Background Image"><img class="lazy-load" data-src="' + picture["thumb"] + '" alt="HD Wallpaper | Background Image ID:' + picture["id"] + '"></a></div><div class="boxcaption"><span class="thumb-info-big"><span>' + picture["resolution"] + '</span>&nbsp;&nbsp;<a href="/by_category/' + $category["id"] + '/1" title="' + $category["name"] + ' HD Wallpapers | Background Images">' + $category["name"] + '</a>&nbsp;&nbsp;<a href="by_subcategory/' + picture["subcategory"]["id"] + '/1" title="' + picture["subcategory"]["name"] + ' HD Wallpapers | Background Images">' + picture["subcategory"]["name"] + '</a></span><br/><div class="overlay"><div><span title="Download Wallpaper" class="btn btn-primary btn-block download-button" data-id="' + picture["id"] + '"><i class="el el-download-alt"></i></span></div><div><span class="btn btn-user" onClick="linkToProfile(' + picture["uploader"]["id"] + '); return false;" title="' + picture["uploader"]["username"] + '&#039;s Wallpaper Submissions"><img class="lazy-load user-avatar" data-src="' + picture["uploader"]["avatar"] + '" alt="">&nbsp;<span class="align-middle">' + picture["uploader"]["username"] + '</span></span></div></div><div class="thumb-stats"><span title="Views"><i class="el el-eye-open"></i> ' + picture["view"] + ' </span>&nbsp;&nbsp;&nbsp;<span title="Download"><i class="el el-download-alt"></i>' + picture["download"] + '</div></div></div><div class="tags-info">';
                                $.each(picture["tag"],function(key, tags) {
                                    $picturehtml += '<a href="/by_tag/' + tags["tag_id"] + '" title="' + tags["name"] + 'HD Wallpapers | Background Images">' + tags["name"] + '</a>&nbsp; &nbsp;';
                                });
                                $picturehtml += '</div></div>';
